...
const Home = () => {
    ...
    const [linkNext , setLinkNext] = useState(`http://141.8.195.239/api/product/list/?page=1`)
    const [linkPrevious , setLinkPrevious] = useState(`http://141.8.195.239/api/product/list/?page=1`)
    ...
    useEffect(() => {
        axios.get(`http://141.8.195.239/api/product/list/?page=1`, {
            headers: {
    ...
    useEffect(() => {
        axios.get(`http://141.8.195.239/api/category/list/`, {
            headers: {
                "Authorization": `Bearer ${tokenRefresh}`
            }
        })
    ...
    useEffect(() => {
        axios.post("http://141.8.195.239/api/token/refresh/", {
    ...
    const SendAnApplication = (ID) => {
      axios.post("http://141.8.195.239/api/product/inquiry/create/", {
          product:ID,
    ...
    const searchCard = (el) => {
        ...
        axios.get(`http://141.8.195.239/api/product/search/?search=${value}`, {headers: {
                "Authorization": `Bearer ${tokenRefresh}`
            }
        })
            .then((res) => {
    ...
    const BarCode = (el) => {
        ...
        if (is_active) {
            if (value.length > 12) {
                axios.get(`http://141.8.195.239/api/product/search/?search=${value}`, {
                    headers: {
                        "Authorization": `Bearer ${tokenRefresh}`
                    }
                })
    ...
    const filter = ({id}) => {
        axios.get(`http://141.8.195.239/api/category/${id}/search/product/` ,{
            headers: {
                "Authorization": `Bearer ${tokenRefresh}`
            }
        })
    ...
    const Analogue = (ID) => {
        analogue.id > 0 ? setAnalogue([]) : axios.get(`http://141.8.195.239/api/product/list/analogue/${ID}`, {
            headers: {
                "Authorization": `Bearer ${tokenRefresh}`
            }
        })
